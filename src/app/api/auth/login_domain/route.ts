...
import {
  getAuthApplicationBaseUrl,
  getAuthDomain,
  getPmquanlyApiKey,
} from '@/lib/server/pmquanly-env';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${getAuthApplicationBaseUrl()}/auth`, {
      method: 'POST',
      headers: {
        Accept: '*/*',
    ...
    console.error('[api/auth/login_domain]', err);
    return NextResponse.json(
